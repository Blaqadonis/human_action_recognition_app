...
8. Schedule a deployment using CRON    ```python batch_deploy.py```
   ...
   I created a separate file which is basically ```batch.py``` but with extra ***razzmatazz mixed with a likkle bit of extra sauce***. This file ```custom_batch.py``` can send you notifications straight to your inbox about the status of the run. It also contains a 
